...
void load_schemas_for_db(const char* dbfile) {
  char* loaded = NULL;
  /* Always load schemas from the .db file (embedded schema blob) */
  if (dbfile) {
...
/* We do NOT use a JSON sidecar in the browser. Schemas are persisted into
   the .db file itself by calling save_schemas_for_db(pager->filename).
   These helpers are therefore minimal: */
static void ems_save_schemas(void) {
  /* no-op: callers should call save_schemas_for_db(table->pager->filename) */
}
static void ems_load_schemas(void) {
  /* no-op: load_schemas_for_db(dbfile) is used from db_open to read embedded schemas */
}
...
static void ems_fs_init(void) {
  if (!emsfs_mounted) {
    ...
    ems_sync_from_idb();
    /* We do NOT load schemas here from a JSON file — db_open will call
       load_schemas_for_db which reads the embedded schema blob from the .db file. */
    emsfs_mounted = 1;
  }
}
